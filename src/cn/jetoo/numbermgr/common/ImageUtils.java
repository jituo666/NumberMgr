...
public class ImageUtils {
    ...
    public static Bitmap getReflectionBitmap(Bitmap src) {
        ...
        Matrix matrix = new Matrix();
        matrix.preScale(1, -1);
        Bitmap reflectionBmp = Bitmap.createBitmap(src, 0, height / 2, width, height / 2, matrix, false);

        Bitmap target = Bitmap.createBitmap(width, (height + height / 2), Config.ARGB_8888);
        Canvas canvas = new Canvas(target);
    ...
    public static boolean saveImageToFile(Bitmap image, String filepath) {
        try {
            ...
            LogHelper.w(TAG, "Failed to save image file: " + e);
        }
        return false;
    }
}
